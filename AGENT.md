...
> Version 0.4 — backup/restore, terminal (terminado) added  
...
│   │   │   ├── Backup.tsx        # Backup/restore via GitLab Packages
│   │   │   └── Terminal.tsx      # Browser terminal via terminado + xterm.js
│   │   └── components/
...
class Settings(BaseSettings):
    ...
    # Diagnostics — targets
    DIAG_GITHUB_URL: str = "https://api.github.com"
    DIAG_PROXY_TEST_URL: str = "https://www.google.com"
    DIAG_EXTRA_HOSTS: list[str] = []    # ["registry.internal:5000"]

    # Backup to GitLab Packages
    GITLAB_URL: str = "https://gitlab.com"
    GITLAB_PROJECT_ID: str = ""          # project with Packages registry
    BACKUP_PATHS: list[str] = []          # enabled items from config file
    BACKUP_RETENTION_COUNT: int = 10     # keep last N backups
    BACKUP_MAX_SIZE_MB: int = 1024    # reject if exceeds this
    TERMINAL_PORT: int = 8022             # terminal WebSocket port (on-demand spawn via terminado)
    TERMINAL_INACTIVITY_TIMEOUT: float = 300  # seconds before auto-terminate
...
| `github` | `httpx.get("https://api.github.com", timeout=5)` — check 200 + x-github-request-id header |
...
```python
# GET /api/disk
# GET /api/disk?force=true  # bypass cache, trigger fresh scan
#
# Response schema:
{
  "mounts": [
...
**Caching and HTTP-level validation:**
...
class DiskCache:
    def __init__(self):
        ...
        # Coalescing: prevent duplicate du processes
        self._in_flight: asyncio.Event = asyncio.Event()
        self._in_flight.set()
        self._refresh_task: asyncio.Task | None = None
        self._stale: bool = False

    async def get_or_fetch(self, force: bool = False) -> tuple[dict, str, float, bool]:
        # Return (data, etag, last_modified, stale)
        # 1. If cache fresh and not force: return cached immediately
        # 2. If another request is already fetching: wait on semaphore instead of spawning new du
        # 3. Otherwise: run du in ThreadPoolExecutor, update cache

    def compute_etag(self, data: dict) -> str:
        # SHA256 hash of (mount paths + top_dirs paths + sizes) for cache validation
        raw = json.dumps({(m["path"], tuple(d["path"] for d in m["top_dirs"])) for m in data["mounts"]}, sort_keys=True)
        return hashlib.sha256(raw.encode()).hexdigest()[:16]
```

- `GET /api/disk` returns headers: `ETag: "<hash>"`, `Last-Modified: <unix timestamp>`
- Client passes `If-None-Match: "<hash>"` or `If-Modified-Since: <timestamp>` — return `304 Not Modified` if unchanged
- `?force=true` bypasses cache entirely, triggers fresh `du` scan
- If cache is stale (>60s old), serve stale data immediately with `"stale": true` and start background refresh

`top_dirs` is computed by running `du -x --max-depth=3 <mount> --block-size=1` as a subprocess. To prevent multiple slow `du` runs:
- **Coalescing**: Use an `asyncio.Event` to track in-flight requests. If a request arrives while `_in_flight` is set, the second caller waits on the event instead of spawning a new subprocess.
- After `du` completes, set `_in_flight` back so subsequent requests trigger a new scan.
...
- "Run all checks" button: `GET /api/diag?force=true` (bypasses cache, runs all checks)
- Individual re-check: `GET /api/diag/{check_name}?force=true` (coalesces with any in-flight check)
- HTTP caching: supports `ETag` and `If-None-Match` — returns `304 Not Modified` if cache is fresh (<30s)
...
- **GitLab Setup** tab:
  - If not configured: show GitLab URL, Project ID, Token inputs
  - "Save" button stores to `~/.config/devpod-cc/gitlab.json`
  - On save: verify token works via GET /projects/{id}/packages (test call)
  - Show error if invalid

- **Backup Items** tab:
  - Load from `~/.config/devpod-cc/backup.yaml`
  - Display: each item name, paths list, "Test" button
  - "Add Item" button: name + path(s) inputs
  - "Save" button publishes to config file
  - "Test All" → shows valid paths + total size estimate

- **Backups** tab:
  - On mount: fetch from GitLab Packages API
  - "Create Backup" button → streams progress via SSE job
  - List: size, date, Restore button, Delete button
  - Badge: "Keeping last N backups"

- **Restore flow**:
  - Click Restore → confirmation modal
  - On confirm → streams extraction via SSE job
  - Shows: extracted file count, any skipped (missing paths)
...
server {
    ...
    # Terminal WebSocket — on-demand terminado
    location /term {
...
micromamba run -n $ENV_NAME pip install \
  fastapi uvicorn[standard] psutil httpx pydantic-settings terminado
...
## 4.10 Backup/Restore (`lib/gitlab.py` + `routers/backup.py`)

### Config file format

User defines backup items in `~/.config/devpod-cc/backup.yaml`:
...
GitLab token stored in `~/.config/devpod-cc/gitlab.json` (0600 permissions):
...
### `lib/gitlab.py` — GitLab Packages API wrapper

```python
import httpx, json, subprocess, asyncio
from pathlib import Path

GITLAB_CONFIG_FILE = Path("~/.config/devpod-cc/gitlab.json").expanduser()
BACKUP_CONFIG_FILE = Path("~/.config/devpod-cc/backup.yaml").expanduser()

@dataclass
class BackupItem:
    name: str
    paths: list[str]  # expanded paths
...
async def list_backups(config: dict) -> list[BackupManifest]:
    # GET /projects/{id}/packages?package_type=generic
    # Filter to packages matching {POD_NAME}-*
    ...

async def create_backup(name: str, items: list[BackupItem], job_id: str) -> str:
    # 1. Create tar.gz locally in /tmp/
    # 2. Check size < BACKUP_MAX_SIZE_MB
    # 3. Upload via PUT /projects/{id}/packages/generic/{package_name}/{version}/{filename}
    # 4. Clean up local tar.gz
    # 5. Enforce retention: delete old versions if count > BACKUP_RETENTION_COUNT
    ...

async def restore_backup(config: dict, package_name: str, job_id: str):
    # 1. Download tar.gz from GitLab Packages
    # 2. Extract to original paths (creating dirs as needed)
    # 3. Clean up downloaded file
    ...

async def delete_backup(config: dict, package_name: str):
    # DELETE /projects/{id}/packages/generic/{package_name}
    ...
```

### API endpoints (`routers/backup.py`)

```
GET  /api/backup/config         → load enabled items from backup.yaml
POST /api/backup/config      → save enabled items to backup.yaml
POST /api/backup/config/test → validate paths exist, return total size estimate

GET  /api/backup             → list backups from GitLab Packages
POST /api/backup             → create backup tar.gz, upload to GitLab (returns job_id)
GET  /api/backup/{name}/restore → download and extract (returns job_id)
DELETE /api/backup/{name}     → delete from GitLab

POST /api/backup/gitlab      → store GitLab credentials to config file
GET  /api/backup/gitlab      → check if configured (returns {configured: bool} only, never exposes token)
DELETE /api/backup/gitlab  → remove GitLab credentials
```
...
class CreateBackupRequest(BaseModel):
    item_names: list[str] = []  # empty = all enabled items

class CreateBackupResponse(BaseModel):
    job_id: str
    backup_name: str  # e.g., "devpod-user42-2024-06-20-143022.tar.gz"
...
class BackupInfo(BaseModel):
    name: str           # "devpod-user42-2024-06-20-143022"
    size_mb: float
    created_at: str     # "2024-06-20T14:30:22Z"
    item_count: int
...
1. List all backups matching `{POD_NAME}-*`
2. Sort by `created_at` descending
3. Delete oldest if count > `BACKUP_RETENTION_COUNT`

### Frontend — `Backup.tsx` panel

- **GitLab Setup** tab (shown if not configured):
  - GitLab URL input
  - Project ID input
...
- **Backup Items** tab:
  - List each enabled item with paths
  - "Add Custom Item" button: name + path input
  - "Remove" button per item
  - "Test Paths" button → shows which exist + total size

- **Backups** tab:
  - "Create Backup" button → streams progress via SSE job
  - List available backups: name, size, date, Restore button, Delete button
  - Retention indicator: "Keeping last 10" badge

- **Restore** flow:
  - User clicks Restore on a backup → confirmation modal: "This will overwrite existing files. Continue?"
  - On confirm → streams via SSE job, shows extracted files
  - On completion → success/error banner

### 5.10 Terminal Panel

- On mount: GET /api/terminal/status → check if running
- "Start Terminal" button if not running
- When running: connect xterm.js to WebSocket at `/term`
- Ping every 30s via POST /api/terminal/ping to keep alive
- Auto-stop button always visible
- Auto-terminate after 5 min inactivity (server-side)
...
const ws = new WebSocket(`wss://${host}/term`)
ws.onmessage = (e) => term.write(e.data)
term.onData = (data) => ws.send(data)
```

- xterm.js addons: fit, webgl (for weak GPU on Citrix)
- Handle reconnect on disconnect (exponential backoff)

---

## 4.11 Terminal (`lib/terminal.py` + `routers/terminal.py` + frontend)

On-demand spawn using **terminado** (pure Python WebSocket terminal). No external binaries needed — all via pip.

### `lib/terminal.py` — terminado wrapper

```python
import asyncio
from terminado import TerminatedManager
from pathlib import Path

class TerminalManager:
    def __init__(self):
        self._manager: TerminatedManager | None = None
        self._port: int = 8022
        self._last_activity: float = 0
        self._inactivity_timeout_sec: float = 300

    async def start(self) -> int:
        await self.stop()
        self._manager = TerminatedManager(
            command=["bash", "--login"],
...
### API endpoints (`routers/terminal.py`)

```
GET  /api/terminal/status    → {running: bool, port: 8022}
POST /api/terminal/start   → start terminado, returns {port: 8022}
POST /api/terminal/stop    → stop terminado
POST /api/terminal/ping   → reset inactivity timer (called periodically by frontend)
```

### Frontend — `Terminal.tsx` panel

```typescript
// Uses xterm.js directly in the browser, connects to terminado WebSocket

// On iframe / embed: connect to nginx /term/ and proxy to terminado WebSocket
```

**Alternative (simpler iframe):**
- terminado serves HTML directly over its port
- Frontend embeds in `<iframe>`
- No xterm.js needed in frontend
- Connect via `wss://devpod-{port}-<pod>.pods.corp/term`

**Recommended: xterm.js approach** — gives full control, better for weak Citrix client than iframe repainting.

### nginx proxy for terminal
...
```python
# In config.py
TERMINAL_PORT: int = 8022
TERMINAL_INACTIVITY_TIMEOUT_SEC: float = 300  # terminate after 5 min idle
```
...
20. **`lib/gitlab.py`** — GitLab Packages API wrapper; test upload/download/delete
21. **`routers/backup.py`** — all backup endpoints; verify token validation and retention enforcement
22. **`Backup.tsx`** — new panel with GitLab config, items editor, backup list, restore flow
23. **`lib/terminal.py`** — terminado spawn wrapper with on-demand process management
24. **`routers/terminal.py`** — start/stop/ping endpoints; verify auto-terminate on inactivity
25. **`Terminal.tsx`** — xterm.js panel with WebSocket connection
26. **nginx config + supervisord config + install script**
27. **End-to-end test** — start everything via supervisord, open browser, verify all panels
...
| `du` takes >10s on large `/projects` | Cache coalescing prevents duplicate runs — serve stale data immediately; run fresh `du` in background. Use `ETag` and `If-None-Match` to avoid unnecessary transfers. |
...
| GitLab token file unreadable | Return 503 with `{"error": "gitlab_not_configured"}` — prompt user to configure via CLI |
| Backup exceeds max size | Check total before creating tar; return 400 with `{"error": "backup_too_large", "size_mb": N, "limit_mb": 1024}` |
...
- **Terminal panel** — xterm.js + WebSocket to a terminado process
